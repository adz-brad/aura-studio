...
import { projectIdState, customBlocksPathState, customBlockIdState } from "./atoms"
import { updatePage, updateItem, deleteCustomBlock, createCustomBlock, copyCustomBlock, useCustomBlock, updateValue, deleteValue, getCustomBlocks } from "@/app/lib/fetch"
...
const MenuInput = ({ block, parentId }) => {
    ...
    let path = `projects/${projectId}/stages/draft/customBlocks/${block?.id}`
    let parentPath = block ? `projects/${projectId}/stages/draft/customBlocks/${parentId}` : null
    ...
    const createBlock = async () => {
        const res = await createCustomBlock(projectId,block?.id,block.blocks ? block.blocks.length : 0)
        updateBlockNav('open',{id:res,title:'New Block',parent:block.id})
    }

    const removeBlock = async () => {
        await deleteCustomBlock(projectId,block.id,block.parent)
    ...
    const reorder = ({source: { index: srcIndex }, destination: { index: destIndex }}) => {
        if(srcIndex !== destIndex){
            ...
            reordered.map((e,i) => {
                updateItem(`projects/${projectId}/stages/draft/customBlocks/${e.id}`, {'order': i})
            })
        }
      }
      ...
      useEffect(() => {
        if(block?.id){
            const col = query (collection(db, "projects", projectId, "stages", "draft", "customBlocks" ), where("parent", "==", block.id))
                ...
                <>
                    <div className="flex flex-col grow space-y-6 overflow-y-auto overflow-x-hidden mb-4 pr-2 twScroll" style={{height: height-262}}>
                        ...
                        <SimpleSelect 
                            ...
                            onChange={(e) => {
                                block?.type === "custom" ?
                                useCustomBlock(projectId,e.target.value,block?.order,block?.id,true,block?.id)
                                ...
                                [
                                    ...
                                    }, 
                                    {
                                        ...
                                        groupOptions: [
                                            ...
                                            {name: 'Drawer Content', value: 'DrawerContent'},
                                            {name: 'Drawer Body', value: 'DrawerBody'},
                                            {name: 'Drawer Header', value: 'DrawerHeader'},
                                            {name: 'Drawer Footer', value: 'DrawerFooter'},
                                            {name: 'Drawer Overlay', value: 'DrawerOverlay'},
                                            {name: 'Drawer Close Button', value: 'DrawerCloseButton'},
                                        ]
                                    },
                                    {
                                        group: 'Menus',
            ...
            <Draggable key={e.id} draggableId={e.id} index={i}>
                ...
                >
                    ...
                    <button 
                            ...
                            type="button" 
                            onClick={() => copyCustomBlock(projectId,e.id,subBlocks.length,block?.id)}
                        >
                            <PiCopySimple />
                        </button>
                </li>

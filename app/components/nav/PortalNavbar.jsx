...
import { account, social } from "@/app/data/menus";
import UserMenu from "./UserMenu";

const PortalNavbar = ({ user, projects }) => {
    ...
    <nav className="absolute top-0 left-0 w-full h-[80px] p-2 rounded-lg">
      ...
      <Link 
          ...
          <div className="flex flex-col">
            <span
              className="font-brand text-2xl font-bold tracking-wide leading-none"
            >
              aura
            </span>
            <span
              className="text-sm font-brand leading-none"
            >
              Studio
            </span>
          </div>
        ...
        <div className="relative flex flex-row items-center space-x-2 h-full ml-auto leading-none">
          ...
          >
            ...
            <div 
              className={`fixed top-[82px] ${userMenu ? 'right-3' : '-right-[300px]'} w-[300px] bg-base-50 dark:bg-base-950 transition-all duration-300 rounded-lg shadow-lg p-2 z-10`}
            >
                                ...
                                return (
                                  ...
                                  >
                                    <Link 
                                      title={link.title}
                                      href={`/studio/${link.id}`}
                                      className="flex hover:text-brand-500 duration-200"

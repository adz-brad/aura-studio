...
import RenderMenu from "@/app/components/studio/RenderMenu"
  ...
  if(!projectId) {
    ...
    useEffect(() => {
      if(projectId && !querySnap){
          const themeCol = query(collection(db, "projects", projectId, "stages", "draft", "theme"))
          const unsub = onSnapshot(themeCol,(snap) => setQuerySnap(snap.docs))
          return () => unsub()
      }
    }, [ projectId ])
    ...
    return (
      <Chakra theme={chakraTheme}>
        ...
        : null}
        <RenderMenu projectId={projectId} menuId={menuId} />
      </Chakra>
    )
  }
}
